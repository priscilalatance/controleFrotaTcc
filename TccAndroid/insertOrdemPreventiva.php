...
if(isset($_REQUEST['placaVeiculo']) && isset($_REQUEST['email']) && isset($_REQUEST['senha'])) {
    ...
    if ($resultadoConsulta->num_rows > 0) {
        ...
        $sqlInsert = "INSERT INTO ordemservico (placa, dataCheck, tipoManutencao, statusOrdem, re) 
                      VALUES ('$placaVeiculo', '$dataCheck', 'Preventiva', '$statusOrdem', '$reUsuario')";

        if ($conn->query($sqlInsert) === TRUE) {
            // Obtém o ID da última ordem de serviço inserida
        ...
        // Separar os números
        $numeros = explode(",", $listaKmManutencao);
