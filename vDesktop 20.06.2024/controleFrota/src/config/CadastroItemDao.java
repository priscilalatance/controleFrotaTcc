...
public class CadastroItemDao {
    ...
    public int salvar(CadastroItem cadItem) {
        int status;
        try {
            st = conn.prepareStatement("INSERT INTO cadastro_produto(marca, modelo, cor,anoFabricacao, anoModelo,"
                    + "descricaoItem, valorUnitario, pecaUniversal)"
                    + " VALUES(?,?,?,?,?,?,?,?)");
    ...
    public boolean alterar(CadastroItem cadItem) {
        try {
            ...
            st = conn.prepareStatement("UPDATE cadastro_produto SET marca = ?, modelo = ?, cor = ?, anoFabricacao = ?, anoModelo  = ?, descricaoItem  = ?, valorUnitario = ?, pecaUniversal = ?  WHERE idCodigo = ?");
    ...
    public CadastroItem consultar(int idCodigo) {
        try {
            st = conn.prepareStatement("SELECT * FROM cadastro_produto WHERE idCodigo = ?");
    ...
    public CadastroItem consultarDescricao(String descricaoItem) {
        try {
            st = conn.prepareStatement("SELECT * FROM cadastro_produto WHERE descricaoItem = ?");
    ...
    public List<CadastroItem> consultarPorDescricao(String descricao) {
        ...
        ResultSet rs = null;
        try {
            st = conn.prepareStatement("SELECT * FROM cadastro_produto WHERE descricaoItem LIKE ?");
